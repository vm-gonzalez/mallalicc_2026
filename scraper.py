...
from app import malla_data  # Importamos la malla directamente de tu app
    ...
    for semestre, cursos in malla_data.items():
        for curso in cursos:
            ...
            if len(partes) > 1:
                ...
                # Pequeña pausa para no saturar el servidor de la universidad
                time.sleep(0.5) 

    with open('prerrequisitos.json', 'w', encoding='utf-8') as f:
        json.dump(mapa_prerrequisitos, f, indent=4, ensure_ascii=False)
        
    print("¡Archivo prerrequisitos.json generado con éxito!")
